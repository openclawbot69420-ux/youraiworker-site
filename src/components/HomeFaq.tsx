    ...
    <section aria-labelledby="faq-title" className="border-b border-slate-200/70 bg-white">
      ...
      <div className="mx-auto max-w-6xl px-4 py-16">
        ...
        <div className="mt-8 grid gap-4 md:grid-cols-2">
          {FAQ_ITEMS.map((item) => (
            <details key={item.question} className="group rounded-2xl border border-slate-200 bg-slate-50/40 p-5 transition-colors hover:bg-white">
              <summary className="cursor-pointer list-none">
                <div className="flex items-start justify-between gap-4">
                  <h3 className="text-sm font-semibold text-slate-900">{item.question}</h3>
                  <span aria-hidden="true" className="mt-0.5 inline-flex h-6 w-6 shrink-0 items-center justify-center rounded-full border border-slate-200 bg-white text-slate-700 transition-transform group-open:rotate-45">
                    +
                  </span>
                </div>
              </summary>
